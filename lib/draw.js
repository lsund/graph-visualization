...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var drawZone = function (z) {
      ctx.rect(z.minx, z.miny, z.width, z.height);
      ctx.lineWidth = 1;
      ctx.stroke();
    };
    ...
    var draw = function () {
      ...
      window.OBJECT.body.zones.forEach(function (z) {
        drawZone(z);
      });
    ...
    var clear = function () {
      ctx.clearRect(0, 0, window.EXPORTS.panelx, window.EXPORTS.panely);
    };

    window.EXPORTS.draw = draw;
    window.EXPORTS.clear = clear;
